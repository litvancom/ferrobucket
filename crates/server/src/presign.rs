...
//!
//! Implements query-string AWS SigV4 signing (`AWS4-HMAC-SHA256`) for
//! path-style S3 URLs. The generated URLs are accepted by s3s `SimpleAuth`
//! via `v4_check_presigned_url` (RESEARCH.md §Pattern 6).
//!
//! Security notes (T-03-05, T-03-06, T-03-07):
//! - The secret key is never included in the generated URL — only the derived
//!   HMAC signature is.
//! - `X-Amz-Expires` is included in the signed string-to-sign, making
//!   expiry unforgeable.
//! - `signed_headers = "host"` ONLY — never content-type (Pitfall 5).
//! - This module stays in the server crate as a plain `pub fn` and is
//!   never compiled to WASM/browser (DEC-ui-ssr).
...
// ─── SigV4 primitives (harvested from tests/integration.rs lines 83–137) ────
...
/// passed through unchanged. A full RFC 3986 encoder is intentionally avoided
/// to keep the implementation minimal and free of new crate dependencies.
...
/// - `secret_key`: AWS secret access key (never included in the URL).
...
///
/// # Security
/// The secret key drives the HMAC signing-key derivation chain and is never
/// written into the output URL (T-03-07). `X-Amz-Expires` is part of the
/// signed canonical request, making the expiry unforgeable (T-03-06).
pub fn presign_url(
    method: &str,
    host: &str,
    path: &str,
    ...
    // Signing key chain: AWS4{secret} → datestamp → region → "s3" → "aws4_request".
...
mod tests {
    ...
    #[test]
    fn test_presign_url_custom_expires() {
        let url = presign_url(
            "PUT",
            "localhost:9000",
        ...
        assert!(
            ...
            "Custom expiry 3600 not found: {url}"
        );
    }

    #[test]
    fn test_presign_url_credential_contains_access_key() {
        let url = presign_url(
            "DELETE",
            "127.0.0.1:9000",
            "/bucket/key",
        ...
        // Credential must contain the access key (percent-encoded /→%2F).
        assert!(
            url.contains("MYACCESSKEY%2F"),
            "Credential must contain encoded access key: {url}"
        );
    }

    #[test]
    fn test_presign_url_no_content_type() {
        // Presigned URLs must not sign content-type (Pitfall 5).
        let url = presign_url(
            "PUT",
            "127.0.0.1:9000",
            "/b/k",
        ...
        assert!(
            !url.contains("content-type"),
            "Presigned URL must not include content-type: {url}"
        );
    }
